...
func (w *Worker) Process(ctx context.Context) error {
	...
	for _, m := range items {
		deliveryCtx := notify.DeliveryContext(ctx)
		if err := w.Sender.Send(deliveryCtx, m.Topic, m.Payload); err != nil {
			attempt := m.Attempts + 1
			delay := time.Duration(1<<min(attempt, 6)) * time.Second
			_ = w.Outbox.Mark(deliveryCtx, m.ID, attempt, err.Error(), time.Now().Add(delay), false)
			continue
		}
		_ = w.Outbox.Mark(deliveryCtx, m.ID, m.Attempts, "", time.Time{}, true)
	}
	return nil
}
