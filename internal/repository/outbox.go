...
func (r OutboxRepo) Acknowledge(ctx context.Context, id string) error {
	result, err := r.DB.ExecContext(ctx, `UPDATE outbox_messages SET sent_at=? WHERE id=? AND sent_at IS NULL`, ts(time.Now()), id)
	if err != nil {
		return err
	}
