...
func (r IdempotencyRepo) GetByKey(ctx context.Context, key string) (string, error) {
	var response string
	err := r.DB.QueryRowContext(ctx, `SELECT response FROM idempotency_keys WHERE key=? ORDER BY created_at DESC LIMIT 1`, key).Scan(&response)
