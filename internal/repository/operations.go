...
func (r SubmissionRepo) Transition(ctx context.Context, id string, next domain.SubmissionStatus, version int, reviewer string) error {
	res, e := r.DB.ExecContext(ctx, `UPDATE submissions SET status=?,version=version+1 WHERE id=? AND version=?`, next, id, version)
	if e != nil {
		return e
	}
	n, _ := res.RowsAffected()
...
func (r SubmissionRepo) SetReviewMetadata(ctx context.Context, id, reviewer string) error {
	res, err := r.DB.ExecContext(ctx, `UPDATE submissions SET reviewed_at=?,reviewer_id=? WHERE id=?`, ts(time.Now()), reviewer, id)
	if err != nil {
		return err
	}
